...
# 1. Replace build method
$buildRegex = '(?s)  @override\s+Widget build\(BuildContext context\) \{.*?\}\s+#'
$newBuild = '  @override
...
# 2. Add new helper methods if they don't exist
$newMethods = '
...
if ($content.IndexOf("_buildBottomBar") -eq -1) {
    $content = $content.Replace("  void _showPurchaseDialog() {", $newMethods + "`n  void _showPurchaseDialog() {")
}

# 3. Rename/Replace _buildTitleRow and _buildCourseImage
$titleRowRegex = '(?s)  Widget _buildTitleRow\(ThemeData theme\) \{.*?\}\s+Widget _buildCourseImage'
...
$courseImageRegex = '(?s)  Widget _buildCourseImage\(\) \{.*?\}\s+Widget _buildPurchaseButton'
...
# 4. Redesign _buildLessonCard
$lessonCardRegex = '(?s)  Widget _buildLessonCard\(String title, String videoUrl, id, exerciseId, files,.*?\}\s+\)\;\s+\}\s+\}'
